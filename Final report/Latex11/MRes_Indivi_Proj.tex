...
\begin{tikzpicture}[scale=0.90,[every node/.style={font=\large,
  ...
  \path (p1.south)+(0.0,-2.0) \Step{2}{Production};
  \path (p2.south)+(0.0,-2.3) \Step{3}{Post-processing};

  \path (p3.east)+(4.0,0.0) \Step{4}{Sputter Coating};
  \path (p4.south)+(0.0,-2.0) \Step{5}{Image Analysis};


  \path (p5.east)+(4.0,0.0) \Step{6}{  Coating Magnetic Material};
...
\end{tikzpicture}									   %
\caption{Fabrication and Image anlysis. The diagram shows stages of the fabrication (Nanoscribe)
and image analysis (\ac*{SEM}) of this project. The final step (the yellow block in the third column)
has not tried in this project}.					  %
\label{fabrication_diagram}							   %
\end{figure}										   %	
												   %
%%%%%%%%%%%%%% CHANGED%%%%%%%%%%%%%
...
\begin{algorithm}[H]
 \KwData{Velocity ($\bm{V}$), RFT, RSM, BST}
 %\KwResult{how to write algorithm with \LaTeX2e }
 %initialization\;
 \While{ $\bm{V} \neq 0$}{
  Select propulsion method from (RFT, RSM, BST)\;
