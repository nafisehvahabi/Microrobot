...
where the cross product (${\bm{P}(\phi)}\times{\bm{\omega}}$) can be represented in the form of skew-symmetric
martix $(\Delta{\{^{h}\bm{P}(\phi)}\}^{h}\bm{\omega})$.


skew-symmetric matrix \footnote{In mathematics, a square matrix $A$ is called a skew-symmetric if its transpose
is equal to its negative ($A^{T} = -A$).}
