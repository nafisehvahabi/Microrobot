...
actuation method. In order to simplify the model, it was broken down into three 
sub-models; 
...
\end{itemize}
 ...
 The entire model made by combining three sub-models as shown
 in the figure \ref{Simulation framework}.
...
\begin{itemize}
  \item Helix pitch angle
  \item Helix pitch
  \item Helix radius
  \item Helix filament radius
\end{itemize}

As it has shown in the figure \ref{Simulation framework}. The system configration is based on the experiment setup
run by \citeauthor{mahoney2011velocity} however they run an experiment on a mili-rorobot. The table \ref{Simultion model configration} represents the details 
...
\caption{Simultion model configration. The detail of the Helmholts setup
...
compare software for simulation
