...
my study. Thanks for advising me throughout the project, for your patient to proofreading my thesis
 and most importantly your care for our children during my pressure time. This thesis would not 
have been possible without you.

\paragraph{}
  I would like to thanks to my daughter Parmin and my son Ario for always giving me their love and joy.
Finally, my spatial thanks to my parent and my brother for their encouragement and support during the project.
...
Robotic surgery has demonstrated its advantages such as reduced pain and discomfort and 
minimal scaring over invasive surgery. As robotic technolog is moving towards miniturisation, the idea of using
microrobot for medical applications such as cardivascular surgery becomes more attractive~\citep{phil2013robotics}
The potential application of microrobot divided into two main categories; in vivo and in vitro applications.
Some in vivo examples are drug delivery, delivery of hyperthermia for cancer treatment and abation of material.
In vitro applications can be microfluid control and cell characterisation and manuplation~\citep{edd2003biomimetic}.  


\paragraph{}

Magnetically actuated helical microswimmers has been reported as a safe microdevice for biomedical application
~\citep{peyer2013magnetic}. However, there are number of challenges such the issue of biocompatibility and the material 
used to make a microrobot needs to be considered carefully for in vivo application\citep{qiu2014noncytotoxic}.
Research into swimming microrobots showed two primary difficulties are the power source and suitable locomotion method, as
 there are many cells, proteins and fibres in biofluid that prevent the motion of the microrobots~\citep{peyer2013bio}. 
vIn addition, extremely small size of the microrobots and the
...
complex biofluid environment make the design aspect very challenging.  
The design of microrobots depends on their application and the desired task.
...
project, we studed two more propulsion methods for the remotely controlled helical microswimmers
 in dynamic magnetic field, \ac*{RSM} and \ac*{SBT}. An actution method of magnetic microrobot can be either force driven or
torque driven~\citep{qiunanohelices}. Fabrication of microdevice was a considerable challenge for a while ~\citep{qiunanohelices} that has been
overcomed by fabrication methods such as 3D laser lithograph.
...
In addition, the new design is developed and printed for the helix which shows using variable pitch
 in helix design rather than constant pitch to satisfy the fabrication process. 
...
We presented a literature review on different aspects of microrobots in chapter 1.
An overview of the main microrobot designs are summerised in the table \ref{Micro}.  
Section \ref{microDesign} demonstrated the details of the effective parameters on 
microrobot\rq{}s design and optimise them to characterise the new design for 
helix shape microrobot. The major part of this project formed by studying and soliving and 
implementing simulation methods
 which is described in section \ref{simulation}.  
...
Section \ref{fabrication} present a brief history of fabrication techniques and section \ref{microFabric}
 describes the fabrication method is applied in this study. Chater 3 provided the result of thi work in terms of either
simulation and fabrication. Thenumber of considerable issues discussed in chapter 4 and conclusion and potential
future work described in chapter 5. An overview of entire system showed in diagram \ref{System Architecture}
...
For the purpose of this study, the design of microrobots is narrow down to a helical 
tail shape with possible propeller head. Therefore, the key characters of the helix were 
identified and all the design was based on these characters. 
The helical shape microrobot is generally copied the design of the helical 
...
other type of microswimmers \citep{peyer2013bio}.The reason is, a filament needs to 
...
the body of the helix. As microhelix is in the fluid environments, investigating the effect of swimming
 mechanism is another aspect of this implementation. Evaluating the outcome of using different fluid 
environment is part of the simulation. 

\paragraph{Simulation software}
Initially Matlab was selected as simulator software. However, after exploring different aspect
 of simulation, the limitation of the software made it impossible to make a complete simulation
 framework. One of the limitations was failing to bind all the physics are involved in simulation 
modeling. Furthermore, Matlab was incapable to consider all aspect of fluid environment. Therefore, 
...
The result of each parameteres\rq{}s effect on the rotational velocity of the microhelix is
 represented as following. The curve in the figure \ref{RV_pitchAngle} describes the relationship between
the helical angle and rotational velocity. The helix with small helix angle will not result in rotating 
...
%\begin{wrapfigure}{r}{0.5\textwidth}
%  \begin{center}
    \includegraphics[width=1.0\textwidth]{RV_pitchAngle}
...
%\begin{wrapfigure}{r}{0.5\textwidth}
%  \begin{center}
    \includegraphics[width=1.0\textwidth]{RV_helixPitch}
...
%\begin{wrapfigure}{r}{0.5\textwidth}
%  \begin{center}
    \includegraphics[width=1.0\textwidth]{RV_helixRadius1}
...
%\begin{wrapfigure}{r}{0.5\textwidth}
%  \begin{center}
    \includegraphics[width=1.0\textwidth]{RV_filamentRadius}
...
The propulsion models is implemented for microrobot locomotion, \ac*{RFT}, has ignored the 
...
Therefore, in the microhelix with the smaller pitch the interaction between various part of the helix
 ...
 the force and torque in microhelix with incredible small pitch. 
...
To validate the result of these study we need to run an experiment in a laboratory to compare
 the result of simulation and experiment.To validate the result of these study we need to run an
 experiment in a laboratory to compare the result of simulation and experiment. The simulation 
framework is required optimisation in order to simulate microhelix in fluids with different viscosity. 
In addition the simulation cab be focused on the magnetic materials used for coating the microhelix 
and analysed the result of using different magnetic materials on the efficiency of swimming 
microrhelix. The possibility of actuating a microrobot with other power source such as ultrasonic 
can be considered. Moreover, the microrhelix can be used as a rigid tail for different propellers. 
Therefore, there are many potential depends on the application for designing a propellers to 
attached to the helical tail. The ideal locomotion method for microrobot is still needs more investigation. 
