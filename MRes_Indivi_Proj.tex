...
Where $\bm{R}_x(\phi)$ is rotation of a vector in the segment frame ($s$) with the $\phi$ angle around $x$
axe and then will apply $\bm{R}_y(-\theta)$ which rotate the result vector with the $-\theta$ angle around
$y$ axe. The final result is a vector in the helix ($h$) frame. 
