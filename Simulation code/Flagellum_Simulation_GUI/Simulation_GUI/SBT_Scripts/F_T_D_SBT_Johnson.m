function F_T_D_SBT_Johnson(R, a, lambda, L)
%   
% Input arguments: 
...
for j = 1:length(lambda)
        a_lambda(:,j) = a(:)./lambda(j)*4/pi;  %Adjust filament radius to be equivalent to RSM and Lighthill SBT sims
end
...
for i = 1:length(a)
    for j=1:length(lambda)
        for k= 1:length(L)
            [F_J, T_J] = SBT_helical_Johnson(a_lambda(i,j), theta(j), nlength(j,k), res, omega, V);
            force_J(i,j,k)=-sum(F_J)*4*pi*arc_length(j,k);
            torque_J(i,j,k)=sum(T_J)*4*pi*arc_length(j,k);
        end
    end
end
...
for i = 1:length(a)
    for j=1:length(lambda)
        for k= 1:length(L)
            [F_J, T_J] = SBT_helical_Johnson(a_lambda(i,j), theta(j), nlength(j,k), res, omega, V);
            drag_J(i,j,k)=sum(F_J)*4*pi*arc_length(j,k);
        end
    end
end
